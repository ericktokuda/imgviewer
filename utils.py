...
###########################################################
def db_get_nonprocessed_images(conn, methodid):
    ...
    query = '''
    SELECT Image.id from Image INNER JOIN
    (
            SELECT Image.id  FROM Image LEFT OUTER JOIN
            (
                    SELECT imageid FROM Imagemethod INNER JOIN method ON
                    Imagemethod.methodid=Method.id AND Method.id={}
            ) AS Q1
            ON Image.id=Q1.imageid WHERE Q1.imageid IS NULL
    )
    NONPR  ON Image.id=NONPR.id AND Image.corrupted=false;
    '''.format(methodid)
